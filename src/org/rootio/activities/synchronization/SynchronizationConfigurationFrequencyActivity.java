...
public class SynchronizationConfigurationFrequencyActivity extends FragmentActivity {
	...
	/**
	 * Sets the notification text about general guidelines for setting the
	 * multicast address
	 */
	...
	private void lookUpControls() {
		...
		this.startTimeLabel = (TextView) this.findViewById(R.id.synchronization_configure_frequency_starttimelabel_tv);
		this.endTimeLabel = (TextView) this.findViewById(R.id.synchronization_configure_frequency_endtimelabel_tv);
		this.startTimeTableRow = (TableRow) this.findViewById(R.id.synchronization_configure_frequency_starttime_tbr);
		this.endTimeTableRow = (TableRow) this.findViewById(R.id.synchronization_configure_frequency_endtime_tbr);
	}
	 ...
	 * 
	 * @param hour
	 *            Integer representation of the selected hour
	 * @param minute
	 *            Integer representation of the selected minute
	 * @param flag
	 *            flag indicating whether this is the start time or end time
	 */
	 ...
	 * 
	 * @param v
	 *            The view (Table Row) containing start time information
	 *            definition
	 */
	public void onSetStartTime(View v) {
		TimePickerPrompt timePicker = new TimePickerPrompt();
		timePicker.show(this.getSupportFragmentManager(), "Start Time", this, 1);
	}
	 ...
	 * 
	 * @param v
	 *            The view (Table Row) containing end time information
	 *            definition
	 */
	 ...
	 * 
	 * @return Array of frequency units that can be used to define the
	...
	private FrequencyUnit[] getFrequencyUnits() {
		...
		String[][] result = agent.getData(true, tableName, columnsToReturn, null, null, null, null, null, null);
		FrequencyUnit[] frequencyUnitsTmp = new FrequencyUnit[result.length];
		for (int i = 0; i < result.length; i++) {
			frequencyUnitsTmp[i] = new FrequencyUnit(Integer.parseInt(result[i][0]), result[i][1]);
		}
		return frequencyUnitsTmp;
	}

	/**
	 * Renders the spinner that is used to choose a configuration frequency time
	 * unit.
	 */
	private void prepareFrequencyDropdown() {
		this.frequencyUnits = this.getFrequencyUnits();
		ArrayAdapter<FrequencyUnit> frequencyUnitsAdapter = new ArrayAdapter<FrequencyUnit>(this, android.R.layout.simple_spinner_item, frequencyUnits);
		frequencyUnitsAdapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
	 ...
	 * 
	 * @param v
	 *            The view (Save button) that was clicked
	 */
	...
	/**
	 * Handles the click of the cancel button effectively finishing this
	 * activity
	 * 
	 * @param v
	...
	/**
	 * Prompts the user for confirmation to alter the configuration, upon which
	 * the configuration is persisted
	 */
	private void promptConfigurationChange() {
		new AlertDialog.Builder(this).setIcon(R.drawable.questionmark).setMessage("Change current configuration?").setTitle("Confirmation").setPositiveButton("Ok", new DialogInterface.OnClickListener() {

			@Override
			public void onClick(DialogInterface dialog, int which) {

				// get the quantity of units
				synchronizationConfiguration.setUnitId(frequencyUnits[spinner.getSelectedItemPosition()].getId());

				// get the id of the unit to be used
				synchronizationConfiguration.setQuantity(Integer.parseInt(quantityEditText.getText().toString()));

				// whether to sync during particular times
				synchronizationConfiguration.setSyncDuringParticularTimes(syncDuringParticularTimesCheckedTextView.isChecked());

				// starttime for the sync
				synchronizationConfiguration.setSyncStartTime(startTimeTextView.getText().toString());

				// end time for the sync
				synchronizationConfiguration.setSyncEndTime(endTimeTextView.getText().toString());
	...
	/**
	 * Updates the screen to reflect the status of the configuration as
	 * persisted in the database.
	 * 
	 * @param synchronizationConfiguration
	 *            A SynchronizationConfiguration object representing the current
	...
	/**
	 * Handles check actions for the option to synchronize during particular
	 * times
	 * 
	 * @param v
	 *            The view (CheckedTextView) That was clicked
	 */
	 ...
	 * @param isInit
	 *            Whether the change of check status is due to user click or to
	 *            initialization to persisted state. True: persisted state,
	 *            False: user click
	 * @param initialValue
	 */
	...
	/**
	 * Toggles the state of the views pertinent to synchronization during a
	 * particular time window, dependent on whether the option is enabled or not
	 * 
	 * @param enabled
	 *            Boolean representing whether the controls are enabled or not.
